        ...
        try: 
            ...
            for i,dat in enumerate(portname.encode('ascii')):
                if sys.version_info[0] < 3:
                    portbuffer[i] = ctypes.c_uint8(int(dat.encode('hex'),16))
                else:
                    portbuffer[i] = int(dat)
...
#        print(configDict)
        ...
        if ret == 0:
            ...
            raise IOError("Failed to open serial port {:d}!".format(ret))
...
class PyUART():
    ...
    def OpenPort(self,ini_name = "CameraSerialConfig.ini",manual_port=None,manual_baud=None):
        ...
        # if os = windows, in retarded python
        # TODO figure out wtf portnum is used for
        if "COM" in portname:
