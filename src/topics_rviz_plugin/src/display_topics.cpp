...
void DisplayTopics::topics()
{
  ...
  ++row;
  std::sort(supported_topics.begin(), supported_topics.end(), [](ros::master::TopicInfo a, ros::master::TopicInfo b)
  {
  ...
  for (auto button : topic_buttons)
  {
    ...
    displayed_topics_.insert(std::pair<std::string, TopicDetails>(button->objectName().toStdString(),
                             details));
  }
